...
function BatchLoaderUnk.create(data_dir, batch_size, seq_length, padding, max_word_l, ignoreTrainSet)
    local self = {}
    ...
    for split, data in ipairs(all_data) do
       if (ignoreTrainSet == 1) and (split == 1) then
	data = data:sub(1, batch_size * seq_length * 100) --make trainSet very small
       end
