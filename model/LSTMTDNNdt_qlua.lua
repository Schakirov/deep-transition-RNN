...
function LSTMTDNNdt_qlua.lstmtdnndt_qlua(rnn_size, n, ndt, dropout, word_vocab_size, word_vec_size, char_vocab_size, char_vec_size,
	 			     feature_maps, kernels, length, use_words, use_chars, batch_norm, highway_layers, hsm, deepOut, deepOutdim, deepOutDropout, deepOutNonLin, up2down)
...
--	local prev_c = outputs[L*2-1]
 --     end
  ...
  -- set up the decoder
    ...
    if deepOutDropout > 0 then -- even if deepOut=0 we need deepOutDropout if we want dropout here (it's unlikely we need it when no deepOut)
