...
function LSTMTDNNdtSeq.lstmtdnndtseq(rnn_size, n, ndt, dropout, input_size, output_size, 
...
--	top_h = nn.Dropout(0.5)(top_h)
	...
	if batch_norm == 1 then	
	  top_h = nn.BatchNormalization(deepOutdim[(deepOut+1)])(top_h) -- normalizes each sequence in batch independently, #x[1] = num of elements in a sequence
        ...
        proj = nn.Linear(deepOutdim[(deepOut+1)], output_size)(top_h)
      else
        proj = nn.Linear(rnn_size, output_size)(top_h)
      end
