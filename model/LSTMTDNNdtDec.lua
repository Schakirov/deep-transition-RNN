...
function LSTMTDNNdtDec.lstmtdnndtdec(rnn_size, n, ndt, dropout, input_size, word_vocab_size, use_words
	 			     batch_norm, highway_layers, hsm, deepOut, deepOutdim, deepOutDropout, deepOutNonLin, up2down) 
    ...
    -- input_size   word_vec_size = dimensionality of word embeddings
    ...
    table.insert(inputs, nn.Identity()()) -- batch_size x word length (char indices)  -- this nn.Identity()() is for sequence vector
    for L = 1,n do
 ...
 --     if Ldt == Ldt then
  --      print("\n in cycle \n")
      ...
      local prev_c_up = {}
        if (Ldt == 1) then
	  prev_h = inputs[L*2+1] --there would be init_layer inputs to here
	  prev_c = inputs[L*2]
	  if up2down then
	    prev_h_up = inputs[n*2+1] --from the uppest layer
	    prev_c_up = inputs[n*2]
	  end
	else
	  prev_h = outputs[L*2]
...
--	top_h = nn.Dropout(0.5)(top_h)
	...
	if batch_norm == 1 then	
	  top_h = nn.BatchNormalization(deepOutdim[(deepOut+1)])(top_h) -- normalizes each sequence in batch independently, #x[1] = num of elements in a sequence
        ...
        proj = nn.Linear(deepOutdim[(deepOut+1)], word_vocab_size)(top_h)
      else
        proj = nn.Linear(rnn_size, word_vocab_size)(top_h)
      end
